...
void add_insure_team(SQLHENV *dbhenv, SQLHDBC *dbhdbc){
	...
	//输入保险单号
	printf("\n                                                 > 保险单号：");
	fgets(insure_number, MAX_LENGTH, stdin);
	fflush(stdin);
	process_string(insure_number, MAX_LENGTH);

	if (strcmp(insure_number, "E") == 0)
		return;
	...
	ret = SQLPrepare(dbhstmt, (SQLCHAR *)"INSERT INTO Insure_Team VALUES(?, ?);", SQL_NTS);
	if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO){
		SQLBindParameter(dbhstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, MAX_LENGTH, 0, insure_number, 0, NULL);
...
void delete_insure_team(SQLHENV *dbhenv, SQLHDBC *dbhdbc){
	...
	/*预编译带参数的SQL语句，并进行参数绑定*/
	ret = SQLPrepare(dbhstmt, (SQLCHAR *)"DELETE FROM Insure_Team WHERE team_number = ? AND insure_number = ?;", SQL_NTS);
	if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO){
		ret = SQLBindParameter(dbhstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, MAX_LENGTH, 0, team_number, 0, NULL);
		ret = SQLBindParameter(dbhstmt, 2, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, MAX_LENGTH, 0, insure_number, 0, NULL);
	}
	else{
		printf("参数绑定失败！\n");
		exit(0);
...
void query_insure_team(SQLHENV *dbhenv, SQLHDBC *dbhdbc){
	...
	/*预编译带参数的SQL语句，并进行参数绑定*/
	SQLPrepare(dbhstmt, (SQLCHAR *)"SELECT team_number FROM Insure_Team WHERE insure_number = ?;", SQL_NTS);
	SQLBindParameter(dbhstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, MAX_LENGTH, 0, insure_number, 0, NULL);
...
void insure_team(){
	...
	do{
		...
		switch (choice){
		case 1:
			add_insure_team(&dbhenv, &dbhdbc);
			break;
		case 2:
			delete_insure_team(&dbhenv, &dbhdbc);
			break;
		case 3:
			query_insure_team(&dbhenv, &dbhdbc);
			break;
		case 0:
			break;
		}
